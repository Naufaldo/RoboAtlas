  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 space-y-8">
      ...
      <div className="p-5 rounded-2xl glass-panel space-y-4 shadow-xl">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-3 rounded-xl bg-slate-950/80 border border-slate-700/60 text-xs font-mono text-slate-100 placeholder-slate-500 focus:outline-none focus:border-cyan-500 transition-colors shadow-inner"
          />
        </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 pt-2 border-t border-slate-800/80 text-xs font-mono">
          ...
          <div className="flex items-center gap-1.5">
            <span className="text-slate-400 mr-1 font-semibold">Difficulty:</span>
          ...
          >
            <div>
              <div className="flex items-center justify-between mb-3.5">
                <span className="text-xs font-mono px-2.5 py-0.5 rounded-md bg-cyan-500/10 text-cyan-300 border border-cyan-500/25 font-semibold">
                  {algo.category}
                </span>
              ...
              {algo.keyEquation && (
                <div className="mt-4">
                  <MathBlock latex={algo.keyEquation} title="Core Mathematical Formulation" />
                </div>
              )}

              {algo.paperRef && (
                <div className="mt-3 text-[11px] font-mono text-slate-400 bg-slate-950/70 p-2.5 rounded-xl border border-slate-800/80">
                  <strong className="text-cyan-400 font-semibold">Classical Paper:</strong> {algo.paperRef}
                </div>
              )}
            </div>
            ...
            <div className="mt-6 pt-4 border-t border-slate-800/80 flex items-center justify-between">
              <span className="text-xs font-mono text-slate-400 flex items-center gap-1.5">
                ...
                <span>Deterministic Sim</span>
              </span>
              <Link
                href={algo.path}
                className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg bg-cyan-500/10 hover:bg-cyan-500/20 text-cyan-300 font-mono text-xs font-semibold border border-cyan-500/30 transition-all shadow-sm"
              >
      ...
      {filteredAlgorithms.length === 0 && (
        <div className="text-center py-16 rounded-2xl glass-panel text-slate-400 font-mono text-xs">
