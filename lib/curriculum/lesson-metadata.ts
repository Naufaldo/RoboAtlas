...
export const LESSON_PEDAGOGY_MAP: Record<string, LessonPedagogyData> = {
  ...
  'rigid-body-and-lagrangian-dynamics': {
    learningObjectivesEn: [
      'Formulate kinetic and potential energy scalar fields in generalized coordinates L(q, q_dot) = K - P.',
      'Derive Euler-Lagrange equations of motion d/dt(∂L/∂q_dot) - ∂L/∂q = tau for serial robot manipulators.',
      'Analyze the canonical dynamic equation M(q)q_ddot + C(q, q_dot)q_dot + g(q) = tau and skew-symmetry of M_dot - 2C.',
      'Differentiate between forward dynamics (simulation) and inverse dynamics (feedforward torque control).',
    ],
    learningObjectivesId: [
      'Merumuskan medan skalar energi kinetik dan potensial pada koordinat umum L(q, q_dot) = K - P.',
      'Menurunkan persamaan gerak Euler-Lagrange d/dt(∂L/∂q_dot) - ∂L/∂q = tau untuk lengan robot serial.',
      'Menganalisis persamaan dinamika kanonikal M(q)q_ddot + C(q, q_dot)q_dot + g(q) = tau dan sifat skew-symmetry M_dot - 2C.',
      'Membedakan dinamika maju (simulasi numerik) dan dinamika invers (kompensasi torsi motor).',
    ],
    whyItMattersEn: 'Dynamics provides the physical law of motion required to compute motor torques and prevent actuator overload in high-speed robot maneuvers.',
    whyItMattersId: 'Dinamika menyediakan hukum fisika gerak untuk menghitung torsi motor dan mencegah beban lebih aktuator pada manuver cepat.',
    progressionStepsEn: ['Lagrangian Energy L=K-P', 'Euler-Lagrange Equation', 'Inertia & Coriolis Matrices', 'Forward/Inverse Dynamics'],
    progressionStepsId: ['Energi Lagrange L=K-P', 'Persamaan Euler-Lagrange', 'Matriks Inersia & Coriolis', 'Dinamika Maju/Invers'],
  },
  'zero-moment-point-and-lipm': {
    learningObjectivesEn: [
      'Derive the Zero Moment Point (ZMP) spatial coordinates from multi-body gravitational and inertial moments.',
      'Apply the Golden Stability Criterion: ZMP strictly inside the convex support polygon (p_ZMP ∈ interior(S)).',
      'Solve the Linear Inverted Pendulum Model (LIPM) differential equation under constant CoM height constraint.',
      'Formulate bipedal and quadrupedal gait timing sequences (Walk, Trot, Pace, Gallop).',
    ],
    learningObjectivesId: [
      'Menurunkan koordinat spasial Zero Moment Point (ZMP) dari momen gravitasi dan inersia multi-bodi.',
      'Menerapkan Kriteria Keseimbangan Emas: titik ZMP berada secara ketat di dalam poligon penumpu kaki.',
      'Menyelesaikan persamaan diferensial Linear Inverted Pendulum Model (LIPM) dengan asumsi tinggi CoM konstan.',
      'Merumuskan sekuens waktu pola gaya berjalan bipedal dan quadruped (Walk, Trot, Pace, Gallop).',
    ],
    whyItMattersEn: 'ZMP and LIPM are the universal mathematical principles preventing bipedal humanoids and quadrupeds from falling during dynamic locomotion.',
    whyItMattersId: 'ZMP dan LIPM adalah prinsip matematis universal pencegah robot humanoid dan quadruped terjatuh saat berjalan dinamis.',
    progressionStepsEn: ['ZMP Moment Balance', 'Support Polygon Condition', 'LIPM Inverted Pendulum', 'Quadruped Gait Planning'],
    progressionStepsId: ['Keseimbangan Momen ZMP', 'Kriteria Poligon Penumpu', 'Pendulum Terbalik LIPM', 'Perencanaan Gait Quadruped'],
  },
  'quadrotor-attitude-cascaded-control': {
    learningObjectivesEn: [
      'Model 6-DOF multirotor rigid body flight equations on SE(3) with body thrust and gyroscopic rotor torques.',
      'Formulate the motor thrust mixing and allocation matrix relating rotor speeds to roll, pitch, yaw, and thrust.',
      'Implement the high-rate nested cascaded PID control architecture (Position -> Velocity -> Attitude -> Angular Rate).',
      'Analyze differential flatness and minimum snap trajectory generation for aggressive agile maneuvers.',
    ],
    learningObjectivesId: [
      'Memodelkan persamaan penerbangan benda kaku multirotor 6-DOF pada SE(3) dengan dorongan propeler dan torsi giroskopik.',
      'Merumuskan matriks pencampuran dorongan dan alokasi motor yang menghubungkan RPM rotor ke torsi roll, pitch, yaw, dan gaya dorong.',
      'Mengimplementasikan arsitektur kendali bertingkat cascaded PID (Posisi -> Kecepatan -> Sikap -> Laju Sudut Gyro).',
      'Menganalisis sifat differential flatness dan pembangkitan trajektori minimum snap untuk manuver terbang lincah.',
    ],
    whyItMattersEn: 'Powers commercial and research drone flight controllers (PX4/ArduPilot) to achieve sub-millimeter hovering stability and high-speed acrobatic agility.',
    whyItMattersId: 'Menjadi inti autopilot drone (PX4/ArduPilot) untuk mencapai kestabilan melayang presisi dan kelincahan manuver akrobatik.',
    progressionStepsEn: ['6-DOF Flight Equations', 'Motor Allocation Matrix', 'Cascaded PID Loops', 'Minimum Snap Trajectories'],
    progressionStepsId: ['Persamaan Terbang 6-DOF', 'Matriks Alokasi Motor', 'Hierarki Cascaded PID', 'Trajektori Minimum Snap'],
  },
  'fossen-marine-craft-dynamics': {
    learningObjectivesEn: [
      'Master the standard SNAME 6-DOF subsea motion notation (Surge, Sway, Heave, Roll, Pitch, Yaw).',
      'Formulate Fossen vector equations with Hydrodynamic Added Mass (M_A) and quadratic fluid damping (D(nu)).',
      'Derive restoring forces and metacentric stability conditions (Center of Buoyancy above Center of Gravity).',
      'Design thruster allocation matrices and closed-loop PID depth/heading regulation for AUVs.',
    ],
    learningObjectivesId: [
      'Menguasai notasi gerak bawah air 6-DOF standar SNAME (Surge, Sway, Heave, Roll, Pitch, Yaw).',
      'Merumuskan persamaan vektor model Fossen dengan Massa Tambahan Hidrodinamika (M_A) dan redaman kuadratik fluida.',
      'Menurunkan gaya pemulih hidrostatis dan syarat stabilitas metasentrik (Pusat Apung di atas Pusat Gravitasi).',
      'Merancang matriks alokasi pendorong thruster dan kendali loop tertutup PID kedalaman/heading AUV.',
    ],
    whyItMattersEn: 'Enables subsea robots (AUVs/ROVs) to navigate deep underwater ocean environments where GPS is absent and water density dominates inertia.',
    whyItMattersId: 'Memungkinkan robot bawah air (AUV/ROV) bernavigasi di kedalaman laut tanpa sinyal GPS dengan inersia fluida yang dominan.',
    progressionStepsEn: ['SNAME 6-DOF Notation', 'Hydrodynamic Added Mass', 'Restoring Buoyancy Moments', 'AUV Depth/Heading Control'],
    progressionStepsId: ['Notasi 6-DOF SNAME', 'Massa Tambahan Hidrodinamik', 'Momen Pemulih Gaya Apung', 'Kendali Kedalaman/Heading AUV'],
  },
  'six-dof-arm-kinematics-and-spherical-wrist': {
    learningObjectivesEn: [
      'Master the kinematic architecture of 6-DOF industrial robot arms with spherical wrists (PUMA 560, UR5).',
      'Apply Pieper\'s Criterion to analytically decouple position solving (joints 1-3) from orientation solving (joints 4-6).',
      'Compute wrist center position p_w = p_target - d6 * a and solve analytical arm inverse kinematics.',
      'Enumerate all 8 distinct kinematic configurations (Shoulder Left/Right, Elbow Up/Down, Wrist Flipped/Non-Flipped).',
    ],
    learningObjectivesId: [
      'Menguasai arsitektur kinematika lengan robot industri 6-DOF dengan spherical wrist (PUMA 560, UR5).',
      'Menerapkan Kriteria Pieper untuk mendekopling penyelesaian posisi (sendi 1-3) dan orientasi (sendi 4-6) secara analitik.',
      'Menghitung posisi pusat pergelangan p_w = p_target - d6 * a dan menyelesaikan kinematika invers analitik lengan.',
      'Menganalisis 8 variasi konfigurasi kinematik fisik lengan (Shoulder Kiri/Kanan, Siku Atas/Bawah, Wrist Flipped).',
    ],
    whyItMattersEn: 'Pieper\'s analytical solution computes 6-DOF inverse kinematics in microseconds, enabling ultra-fast real-time industrial trajectory tracking.',
    whyItMattersId: 'Solusi analitik Pieper menghitung kinematika invers 6-DOF dalam mikrodetik, memungkinkan pelacakan trajektori industri waktu-nyata berkecepatan tinggi.',
    progressionStepsEn: ['Spherical Wrist Geometry', 'Wrist Center Decoupling', 'Analytical Joint Angles', '8 Configuration Branches'],
    progressionStepsId: ['Geometri Pergelangan Bola', 'Dekopling Pusat Pergelangan', 'Sudut Sendi Analitik', '8 Cabang Konfigurasi'],
  },
  'trajectory-generation-and-grasp-planning': {
    learningObjectivesEn: [
      'Generate smooth continuous joint space trajectories using Cubic (3rd order) and Quintic (5th order) polynomial splines.',
      'Synthesize time-optimal trapezoidal and jerk-limited S-curve velocity profiles respecting motor limits.',
      'Formulate the Grasp Matrix G mapping contact forces to net object disturbance wrenches.',
      'Differentiate between Form Closure and Force Closure within Coulomb friction cones for antipodal grippers.',
    ],
    learningObjectivesId: [
      'Membangkitkan trajektori ruang sendi mulus dan kontinu menggunakan spline polinomial Kubik (derajat 3) dan Kuintik (derajat 5).',
      'Merancang profil kecepatan trapesium dan kurva-S terbatas jerk sesuai batas aktuator motor.',
      'Merumuskan Matriks Cengkeraman G yang memetakan gaya kontak jari ke gaya resultan pada objek.',
      'Membedakan Form Closure dan Force Closure di dalam kerucut gesek Coulomb untuk gripper antipodal.',
    ],
    whyItMattersEn: 'Guarantees that robot arms manipulate and pick up objects smoothly without violent motor jerking or dropping fragile payloads.',
    whyItMattersId: 'Menjamin lengan robot memindahkan dan mencengkeram objek secara mulus tanpa sentakan motor yang merusak atau menjatuhkan muatan.',
    progressionStepsEn: ['Polynomial Splines', 'Trapezoidal Velocity Profiles', 'Grasp Matrix G', 'Force/Form Closure Friction'],
    progressionStepsId: ['Spline Polinomial', 'Profil Kecepatan Trapesium', 'Matriks Grasp G', 'Gesekan Force/Form Closure'],
  },
  'visual-odometry-and-rgbd-depth-processing': {
    learningObjectivesEn: [
      'Formulate Two-View Epipolar Geometry and the Essential Matrix E = [t]_x R relating camera frames.',
      'Implement the 8-point algorithm with SVD decomposition and Cheirality testing to recover relative 6-DOF camera motion.',
      'Back-project 2D depth pixels into metric 3D point clouds using camera intrinsic matrix K: P = Z * K^-1 [u, v, 1]^T.',
      'Analyze monocular scale ambiguity versus metric scale resolution in RGB-D and stereo visual odometry.',
    ],
    learningObjectivesId: [
      'Merumuskan Geometri Epipolar Dua Sudut Pandang dan Matriks Esensial E = [t]_x R penghubung frame kamera.',
      'Mengimplementasikan algoritma 8-titik dengan dekomposisi SVD dan uji Cheirality untuk mengekstrak pergerakan kamera 6-DOF.',
      'Memproyeksikan balik piksel kedalaman 2D ke point cloud 3D metrik menggunakan matriks intrinsik K: P = Z * K^-1 [u, v, 1]^T.',
      'Menganalisis ambiguitas skala monokuler versus resolusi skala metrik pada odometri visual RGB-D dan stereo.',
    ],
    whyItMattersEn: 'Visual Odometry provides high-rate visual pose tracking for autonomous vehicles, drones, and planetary rovers in GPS-denied environments.',
    whyItMattersId: 'Odometri Visual menyediakan pelacakan pose berkecepatan tinggi untuk kendaraan otonom, drone, dan robot penjelajah tanpa sinyal GPS.',
    progressionStepsEn: ['Epipolar Constraint', 'Essential Matrix SVD', 'RGB-D Point Back-Projection', 'Visual Odometry Pipeline'],
    progressionStepsId: ['Batasan Epipolar', 'SVD Matriks Esensial', 'Proyeksi Balik Point RGB-D', 'Pipeline Odometri Visual'],
  },
  'quaternions-and-numerical-optimization': {
    learningObjectivesEn: [
      'Master Unit Quaternion algebra on S^3: scalar-vector representation, Hamilton product, and Gimbal Lock avoidance.',
      'Apply Spherical Linear Interpolation (SLERP) for smooth constant-angular-velocity 3D orientation trajectories.',
      'Formulate Non-Linear Least Squares residual minimization for robotics state estimation.',
      'Derive and compare Gauss-Newton (J^T J Δx = -J^T r) and Levenberg-Marquardt adaptive damping optimization.',
    ],
    learningObjectivesId: [
      'Menguasai aljabar Kuaternion Satuan pada S^3: representasi skalar-vektor, perkalian Hamilton, dan pencegahan Gimbal Lock.',
      'Menerapkan Interpolasi Linier Bola (SLERP) untuk trajektori orientasi 3D berkecepatan sudut konstan yang mulus.',
      'Merumuskan minimisasi residual Kuadrat Terkecil Non-Linear untuk estimasi status robotika.',
      'Menurunkan dan membandingkan optimasi Gauss-Newton (J^T J Δx = -J^T r) dan redaman adaptif Levenberg-Marquardt.',
    ],
    whyItMattersEn: 'Unit quaternions and non-linear least squares solvers form the core computational engine inside Graph SLAM, ROS, and flight attitude filters.',
    whyItMattersId: 'Kuaternion satuan dan pemecah kuadrat terkecil non-linear adalah mesin inti di balik Graph SLAM, ROS, dan filter sikap drone.',
